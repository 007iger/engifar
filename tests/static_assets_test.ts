...
Deno.test("ported PR visuals use real room data and server-backed private results", async () => {
  ...
  assert.doesNotMatch(source, /answer\.allParticipantsAnswered/);
  assert.match(source, /\/quiz\/questions\/\$\{index\}\/grade/);
  assert.match(source, /RECOVERY_SYNC_INTERVAL_MS = 60_000/);
  assert.doesNotMatch(source, /ACTIVE_QUIZ_SYNC_INTERVAL_MS|20_000/);
  assert.match(source, /syncRequested = true;\s+if \(syncing\) return;/);
  assert.match(source, /while \(syncRequested && !finished\)/);
  assert.match(source, /}, RECOVERY_SYNC_INTERVAL_MS\);/);
  assert.match(source, /eventQueue = eventQueue\.then\(\(\) => applyQuestionStarted\(event\)\)/);
  assert.match(source, /eventQueue = eventQueue\.then\(\(\) => applyQuestionEnded\(event\)\)/);
  assert.match(source, /const retryDelays = \[300, 800\]/);
